Collgroup.Mycollection1 Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: CollGroup.mycollection1.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from CollGroup.mycollection1
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: CollGroup.mycollection1.run
      vars:
        run_x: 42
```
